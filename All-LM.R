...
sigma.p=0.01
diff.res=matrix(0,nrow=length(Sigma.varepsilon)*length(ETA),ncol=6)
###################################
###################################
...
sig=matrix(0,nrow=length(N),ncol=10)
tm=matrix(0,nrow=length(N),ncol=10)
#
####################################
t1=proc.time()
for (i in 1:length(N)) {
  ...
  for (m in 1:M) {
    ...
    L.res[i,1:3]=ifelse(L.bounds[1:3,1]<LM.t & LM.t<L.bounds[1:3,2],
                        L.res[i,1:3]+1,L.res[i,1:3])
    P.res[i,1:3]=ifelse(P.bounds[1:3,1]<LM.t & LM.t<P.bounds[1:3,2],
                        P.res[i,1:3]+1,P.res[i,1:3])
    F.res[i,1:3]=ifelse(F.bounds[1:3,1]<LM.t & LM.t<F.bounds[1:3,2],
                        F.res[i,1:3]+1,F.res[i,1:3])
    T.res[i,1:3]=ifelse(T.bounds[1:3,1]<LM.t & LM.t<T.bounds[1:3,2],
                        T.res[i,1:3]+1,T.res[i,1:3])
    
    sig[i,]=sig[i,]+c(L.sig.s,L.sig.t,P.sig.s,P.sig.t,F.sig.s,F.sig.t,T.sig.s,T.sig.t,LM.sig.s,LM.sig.t)
    tm[i,]=tm[i,]+c(L.s,L.t,P.s,P.t,F.s,F.t,T.s,T.t,LM.s,LM.t)
...
#
res=cbind(L.res,P.res,F.res,T.res)
#
res/M
sig/M
tm/M
